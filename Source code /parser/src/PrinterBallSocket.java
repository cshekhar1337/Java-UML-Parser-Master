...
/**
 * Created by shekhar on 23/10/15.
 */
public class PrinterBallSocket implements JavaListener {
    ...
    PrinterBallSocket(String imagename) {
        filename = imagename;


    }

    public Set<String> getclasswithreferenceofinterface(List<JavaParseTreeBuilder> classList1, String s) {

        Set<String> arr = new HashSet<>();
        for(JavaParseTreeBuilder i : classList1) {
            for (List<String> w : i.list_of_methodReferences)
                i.list_of_classReferences.add(w);


            for (List<String> l : i.list_of_classReferences)
                if (l.get(0).equals(s))
                    arr.add(i.class_entered);
    ...
    public boolean isClass (List<JavaParseTreeBuilder> classList1, String s) {
        for(JavaParseTreeBuilder i : classList1) {
            if(i.class_entered.equals(s))
        ...
        for (JavaParseTreeBuilder i : classList1) {
            ...
            if (i.typeclassorinterface == 1) {
                ...
                for (String j : i.list_of_interfaces) {
                    if (i.typeclassorinterface == 1) {
                        Set<String> arr = getclasswithreferenceofinterface(classList, j);
                        if (arr.isEmpty())
                            plantumlstring += i.class_entered + " -() " + j + "\n";
                        for (String ss : arr)
                            if(isClass(classList, ss))
                                plantumlstring += i.class_entered + " -0)-  " + ss + " :" + j + "\n";
                            else
                                plantumlstring += i.class_entered + " -() " + ss + " :" + j + "\n";
    ...
    public void exitMethodDeclaration(JavaParser.MethodDeclarationContext ctx) {
        ...
        if (parentContext != null && parentContext.getParent() != null) {
            ...
            if (grandParentContext instanceof JavaParser.ClassBodyDeclarationContext) {
                ...
                String s =  temp + ctx.Identifier() + "" + parameter + " : " + m_type;
